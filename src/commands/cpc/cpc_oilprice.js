  ...
  async execute(interaction) {
    ...
    try {
      ...
      const oil_header = new TextDisplayBuilder().setContent(
        [
          `# ${oil_title}`,
          `-# 自 **<t:${parseMonthDayToUnix(
            oil.PriceUpdate
          )}>** 起實施，單位：元/公升`,
        ].join("\n")
      );

      const oil_msg = new TextDisplayBuilder().setContent(
        [
      ...
      const oil_footer = new TextDisplayBuilder().setContent(
        `-# 以上價格僅供參考，實際價格以中油官網為準・資料來源：台灣中油`
      );

      const cpc_logo = new MediaGalleryBuilder().addItems([
        new MediaGalleryItemBuilder().setURL(
          "https://ws.cpc.com.tw/001/Upload/1/sites/pagebackimage/ff201a11-44a7-4d52-970d-b5e747044b4b.png"
        ),
      ]);

      const cpc_url = new ActionRowBuilder().addComponents([
        new ButtonBuilder()
          .setLabel("台灣中油官網")
