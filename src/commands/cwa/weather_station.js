...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription(
      "檢視 氣象站相關資訊(此為實驗功能，僅能檢視 台中龍井 的觀測站)"
    ),
  ...
  async execute(interaction) {
    ...
    const wtResult = await axios.get(
      `https://opendata.cwa.gov.tw/api/v1/rest/datastore/O-A0001-001?Authorization=${process.env.cwa_key}&StationId=C0F9R0`
    );
    const { records } = wtResult.data;

    const station = records.Station[0];
    const locaName =
      station.GeoInfo.CountyName + " " + station.GeoInfo.TownName;
    ...
    const highTemp =
      station.WeatherElement.DailyExtreme.DailyHigh.TemperatureInfo.AirTemperature.toString();
    const lowTemp =
      station.WeatherElement.DailyExtreme.DailyLow.TemperatureInfo.AirTemperature.toString();
